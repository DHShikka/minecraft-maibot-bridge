...
| **饿了没吃的时自己去找** | `mc_forage`（干草块→面包 / 打动物 / 翻宝箱，走不通会自动换下一条） |
...
| `[combat] autoEat` | `true` | **受伤或饿了自动吃东西**，而且按营养值挑**最好的**那份（牛排 > 面包 > 腐肉） |
...
自动进食：牛排（血量 20，饥饿 15，饿了）
```

**饿肚子/带伤会自动吃东西**（`autoEatHealth`/`autoEatHunger` 默认 16）：按营养值挑最好的那份，
同营养看饱和度 —— 背包里同时有牛排、面包、腐肉时会先吃牛排。背包里一点吃的都没有时，
`mc_state` 会直接给出**能照抄的操作序列**（干草块→面包 / 翻宝箱 / 打动物再烤熟 / 种地）。

> 真机实测：饥饿被压到 15 时自动吃了 3 块牛排（面包和腐肉一块没动），饥饿值回到 20；
> 血量 14 时「受伤」那条触发路径也生效了。
